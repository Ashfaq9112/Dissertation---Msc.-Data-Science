...
from gamma_bench.corr_test.corr_test import CorrelationTest
...
def main():
    ...
    for model_name in CFG['models']:
        if model_name not in final_res:
        ...
        for seed in CFG['seeds']:
            ...
            for mth in build_methods():
                ...
                rec = RunRecord(
                    ...
                    perf=PerfMetric# Step 1: compute and STORE the full per-coordinate vectors.
    vecs = {l: self._sensitivity_vec(st) for l, st in stats.items()}

    # Step 2: reduce each vector to a layer scalar for water-filling.
    score = {l: self._reduce_to_scalar(v) for l, v in vecs.items()}s(ppl_wikitext2=ppl_q),
